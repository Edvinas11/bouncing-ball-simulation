...
pygame.display.set_caption("Bouncing Image within a Ball")
...
# Load the image
bouncing_image = pygame.image.load("images/trump.png")
bouncing_image = pygame.transform.scale(bouncing_image, (40, 40))  # Resize image if necessary

# Font initialization
font = pygame.font.Font(None, 36)

class BouncingImage:
    def __init__(self, image, initial_position, radius, velocity):
        self.image = image
...
# Start with one bouncing image
bouncing_image_instance = BouncingImage(bouncing_image, (WIDTH / 2, HEIGHT / 2), radius=20, velocity=[-4, -4])
...
while True:
    ...
    if running:
        ...
        if bouncing_image_instance.check_collision(big_ball_mask, big_ball_rect):
            bouncing_image_instance.bounce()
            bouncing_image_instance.play_collision_note()

        # Draw everything
        screen.fill(BLACK)
        screen.blit(big_ball_image, big_ball_rect.topleft)

        bouncing_image_instance.draw(screen)
