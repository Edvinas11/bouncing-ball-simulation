...
class MiniBall:
    def __init__(self, color, initial_position, radius, velocity):
        ...
        self.is_moving = True
        self.tail = []
        self.elapsed_time = 0  # Track elapsed time using clock
        self.gravity_effect = random.choice([gravity, -gravity])

    def move(self, dt):
        self.prevPos = self.position.copy()  # Keep track of previous position
        
        # Apply random gravity effect
        self.velocity += self.gravity_effect

        self.position += self.velocity
        self.update_tail()
        self.elapsed_time += dt  # Increment elapsed time
...
mini_balls = [MiniBall((color.r, color.g, color.b), (WIDTH / 2 -10, HEIGHT / 2 - 90), radius=20, velocity=[-4, -4])]
particles = []
...
while True:
    ...
    if running:
        ...
        for mini_ball in mini_balls:
            ...
            # Check if the ball has been active for exactly 6 seconds
        ...
        if not mini_balls:
            ...
            mini_balls.append(MiniBall(ball_color, random_point_in_circle(big_ball_radius - 150, big_ball_center), radius=20, velocity=[-4, -4]))
